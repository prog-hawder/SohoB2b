...
class Pesquisa extends StatefulWidget {
  ...
  const Pesquisa({Key? key, required this.query}) : super(key: key);
  ...
  Future<List<Produtos>> fetchProdutos() async {
    ...
    do {
      final response = await http.get(
        Uri.parse(
            '$acesso/snsistemasb2b-api/api/v1/cliente/$codcliente/produtos/buscar-descricao/${widget.query}?limit=15&page=$page'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        allProducts.addAll(
            produtosJson.map((data) => Produtos.fromJson(data)).toList());
